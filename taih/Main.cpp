...
//	FILE* Log = NULL;
//	fopen_s(&Log, "GetBoneName.txt", "w+");

//	TArray< FName > BoneName;

//	target->Mesh->GetBoneNames(&BoneName);

//	for (int i(0); i < BoneName.Count; ++i)
//	{
//		if (!bGetBone) continue;
//		if (i == BoneName.Count) bGetBone = false;

//		fprintf(Log, "Name: %-10s %d\n", BoneName.Data[i].GetName(), i);
...
//	//WCHAR buffer[256];
//	//swprintf(buffer, 256, L"%s", target->Mesh->GetBoneName(14));


//	//for (int i = 0; i < 15; ++i) {
//		//swprintf(buffer, L"%s", target->Mesh->GetBoneName(14));
//		//if (wcslen(buffer) > 0) {
//			myfile << ": " << "Hello!" << std::endl;
//			//myfile << i << ": " << "hello!" << std::endl;
//		//}
//	//}
//	logBones = false;
...
//FVector Head = WorldToScreen(pCanvas, Player->Mesh->GetBoneLocation(Player->Mesh->GetBoneName(14), 0));
